...
- (UIActionSheet *)pageActionSheet {
    ...
    if(!pageActionSheet) {
        ...
        if((self.availableActions & SVWebViewControllerAvailableActionsCopyLink) == SVWebViewControllerAvailableActionsCopyLink)
            [pageActionSheet addButtonWithTitle:NSLocalizedString(@"复制链接地址", @"")];
        
        if((self.availableActions & SVWebViewControllerAvailableActionsOpenInSafari) == SVWebViewControllerAvailableActionsOpenInSafari)
            [pageActionSheet addButtonWithTitle:NSLocalizedString(@"用Safari打开", @"")];
...
- (void)updateToolbarItems {
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        ...
        [self.navigationController.toolbar setTintColor:[UIColor colorWithRed:255.0/255.0 green:128.0/255.0 blue:176.0/255.0 alpha:1.0]];
        
        //[self.navigationController.toolbar setFrame:CGRectMake(0.0f, self.view.frame.size.height - 40.0f, 320.0f, 40.0f)];
        
        if ([[[UIDevice currentDevice] systemVersion] floatValue] < 5.0f) {} else {
            
            [self.navigationController.toolbar setBackgroundImage:[UIImage imageNamed:@"footer_lightpink_44.png"] forToolbarPosition:UIToolbarPositionAny barMetrics:UIBarMetricsDefault];
        }
    }
}
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
	...
	if([title isEqualToString:NSLocalizedString(@"用Safari打开", @"")])
        [[UIApplication sharedApplication] openURL:self.mainWebView.request.URL];
    
    if([title isEqualToString:NSLocalizedString(@"复制链接地址", @"")]) {
